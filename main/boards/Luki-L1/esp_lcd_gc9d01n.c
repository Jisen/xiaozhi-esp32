...
static const gc9d01n_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {0x74, (uint8_t[]){0x05, 0x4d, 0x00, 0x00, 0x01, 0x00, 0x00}, 7, 0},
    {0xB5, (uint8_t[]){0x0D, 0x0D}, 2, 0},
    {0xB6, (uint8_t[]){0x00, 0x00}, 2, 0},
    {0x60, (uint8_t[]){0x38, 0x09, 0x1E, 0x7A}, 4, 0},
    {0x63, (uint8_t[]){0x38, 0xAE, 0x1E, 0x7A}, 4, 0},
    {0x64, (uint8_t[]){0x38, 0x0B, 0x70, 0xAB, 0x1E, 0x7A}, 6, 0},
    {0x66, (uint8_t[]){0x38, 0x0F, 0x70, 0xAF, 0x1E, 0x7A}, 6, 0},
    ...
    {0x6E, (uint8_t[]){0x00, 0x00, 0x00, 0x02, 0x14, 0x12, 0x0C, 0x0A, 0x1E, 0x1D, 0x08, 0x00, 0x16, 0x15, 0x00, 0x00, 0x00, 0x00, 0x15, 0x16, 0x00, 0x07, 0x1D, 0x1E, 0x09, 0x0B, 0x11, 0x13, 0x01, 0x00, 0x00, 0x00}, 31, 0},
    {0x98, (uint8_t[]){0x3E}, 1, 0},
    {0x99, (uint8_t[]){0x3E}, 1, 0},
    {0x9b, (uint8_t[]){0x3b}, 1, 0},
    {0x93, (uint8_t[]){0x33, 0x7f, 0x00}, 3, 0},
    {0x91, (uint8_t[]){0x0E, 0x09}, 2, 0},
    {0x70, (uint8_t[]){0x04, 0x02, 0x0d, 0x04, 0x02, 0x0d}, 6, 0},
    {0x71, (uint8_t[]){0x04, 0x02, 0x0d}, 3, 0},
    {0xc3, (uint8_t[]){0x26}, 1, 0},
    {0xc4, (uint8_t[]){0x26}, 1, 0},
    {0xc9, (uint8_t[]){0x1c}, 1, 0},
    {0xf0, (uint8_t[]){0x02, 0x03, 0x0a, 0x06, 0x00, 0x1a}, 6, 0},
    {0xf2, (uint8_t[]){0x02, 0x03, 0x0a, 0x06, 0x00, 0x1a}, 6, 0},
    {0xf1, (uint8_t[]){0x38, 0x78, 0x1b, 0x2e, 0x2f, 0xc8}, 6, 0},
    {0xf3, (uint8_t[]){0x38, 0x74, 0x12, 0x2e, 0x2f, 0xdf}, 6, 0},
...
static esp_err_t panel_gc9d01n_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y){
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]){gc9d01n->madctl_val}, 1), TAG, "send command failed");
    ESP_LOGW(TAG, "panel_gc9d01n_mirror:REG %02Xh:%02Xh", LCD_CMD_MADCTL,gc9d01n->madctl_val);
    return ESP_OK;
}
...
static esp_err_t panel_gc9d01n_swap_xy(esp_lcd_panel_t *panel, bool swap_axes){
    ...
    ESP_LOGW(TAG, "panel_gc9d01n_swap_xy:REG %02Xh:%02Xh", LCD_CMD_MADCTL,gc9d01n->madctl_val);
